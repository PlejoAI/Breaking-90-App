...
export default function App() {
  ...
  const openLegalLink = (url, title) => {
    if (!url) {
      Alert.alert(title, `${title} link needs to be added before App Store review.`);
      return;
    }
    openLink(url);
  };
  ...
  const startMembershipTrial = async () => {
    if (!legalAccepted) {
      Alert.alert('Agreement Required', 'Please accept the Privacy Policy and Terms of Service before starting your free trial.');
      return;
    }
        ...
        <ScrollView contentContainerStyle={styles.paywallContainer} style={{ width: '100%', alignSelf: 'center' }}>
          ...
          <View style={styles.pricingBox}>
            ...
            <View style={styles.pricingList}>
              <Text style={styles.pricingItem}>✓ AI Swing Critiques with Chad's savage feedback</Text>
              <Text style={styles.pricingItem}>✓ AI Audio Coach that talks you through the fix</Text>
              <Text style={styles.pricingItem}>✓ Frame-by-frame biomechanics breakdown</Text>
              <Text style={styles.pricingItem}>✓ Personalized drill plan for your biggest flaw</Text>
              <Text style={styles.pricingItem}>✓ Custom YouTube drill library</Text>
              <Text style={styles.pricingItem}>✓ Swing history and progress tracking</Text>
              <Text style={styles.pricingItem}>✓ Score tracking to chase breaking 90</Text>
              <Text style={styles.pricingItem}>✓ Virtual caddie for golf questions and strategy</Text>
            </View>
          ...
          <TouchableOpacity style={styles.legalRow} onPress={() => setLegalAccepted(!legalAccepted)} activeOpacity={0.85}>
            <View style={[styles.checkbox, legalAccepted && styles.checkboxChecked]}>
              {legalAccepted && <Text style={styles.checkboxMark}>✓</Text>}
            </View>
            <Text style={styles.legalText}>
              I agree to the{' '}
              <Text style={styles.legalLink} onPress={() => openLegalLink(PRIVACY_POLICY_URL, 'Privacy Policy')}>Privacy Policy</Text>
              {' '}and{' '}
...
const styles = StyleSheet.create({
  ...
  paywallBtnDisabled: { opacity: 0.45 },
  paywallBtnText: { color: '#000', fontSize: 18, fontWeight: '900', letterSpacing: 1 },
  legalRow: { flexDirection: 'row', alignItems: 'flex-start', width: '100%', marginBottom: 18, paddingHorizontal: 4 },
  checkbox: { width: 24, height: 24, borderRadius: 6, borderWidth: 2, borderColor: '#444', marginRight: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: '#090909' },
  checkboxChecked: { backgroundColor: '#00FF66', borderColor: '#00FF66' },
  checkboxMark: { color: '#000', fontSize: 16, fontWeight: '900', lineHeight: 20 },
  legalText: { flex: 1, color: '#AAA', fontSize: 13, lineHeight: 19 },
  legalLink: { color: '#00FF66', fontWeight: '800', textDecorationLine: 'underline' },
  loginBtn: { padding: 10 },
